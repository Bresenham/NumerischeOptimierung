...
nicht erfüllt ist, muss das Verfahren 'resettet' werden. Das Newton-Verfahren (damit auch Quasi-Newton-Verfahren) minimieren eine Funktion
(hier der Gradient). Da der Gradient aber nicht nur in einem Minimum sondern auch in einem Maximum oder Sattelpunkt gleich Null sein kann,
...
Um herauszufinden, ob Methode (1) oder (3) am besten geeignet ist (d.h. die richtig Lösung in der kürzesten Zeit findet),
wird die Laufzeit des inversen BFGS-Verfahrens bestimmt. Da weder bei der Bazaara-Shetty oder Himmelblau Funktion noch
200-dimensionalen Rosenbrock-Funktion das BFGS-Verfahren öfter als ein oder zwei mal in der Reset-Bedingung gelandet ist,
verwende ich für eine bessere Vergleichbarkeit die Funktion aus Aufgabe 7 in leicht abgewandelter Form:

$$ h(1, x_1, x_2, x_3) = x_1\cdot e^{ -(x_2^2 + x_3^2)} \cdot \frac{\text{sinh}(x_3^2)}{x_3^2}$$

mit Startwert $x_0 = [10, 0.05, 1]^T$.
