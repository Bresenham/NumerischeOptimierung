...
bzw. die Iterationsvorschrift (die $k$-Nummerierungen wurden aufgrund Platzmangels weggelassen):

$$(A^{(k+1)})^{-1} = A^{-1} - \frac{ A^{-1}(y-As)\left(\frac{ y-As }{ (y-As)^Ts } \right)^TA^{-1} }{1 + \left(\frac{ y-As }{ (y-As)^Ts }\right)^TA^{-1}(y-As)} - \left(A + \left(y - As\right)\cdot\left( \frac{ y-As }{ (y-As)s } \right)^T\right)^{-1}$$
